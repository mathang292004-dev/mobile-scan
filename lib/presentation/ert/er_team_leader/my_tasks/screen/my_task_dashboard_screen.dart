...
class MyTaskDashboardScreen extends StatefulWidget {
  const MyTaskDashboardScreen({super.key, this.role = 'tl'});

  final String role;

  @override
  State<MyTaskDashboardScreen> createState() => _MyTaskDashboardScreenState();
}

class _MyTaskDashboardScreenState extends State<MyTaskDashboardScreen> {
  final GlobalKey<SearchBarWidgetState> searchBarKey =
      GlobalKey<SearchBarWidgetState>();

  MyTaskDashboardCubit get _cubit => widget.role == 'member'
      ? AppDI.memberTaskDashboardCubit
      : AppDI.myTaskDashboardCubit;

  String get _taskDetailsRoute => widget.role == 'member'
      ? Routes.erTeamMemberTasksScreen
      : Routes.taskDetails;
  ...
  void initState() {
    ...
    if (cubit.state.data == null &&
        cubit.state.processState != ProcessState.error) {
      // First load — send today's date as the initial daterange
      final today = DateTime.now();
      final from =
          '${today.year}-${today.month.toString().padLeft(2, '0')}-'
          '${today.day.toString().padLeft(2, '0')}T00:00:00.000Z';
      final to =
          '${today.year}-${today.month.toString().padLeft(2, '0')}-'
          '${today.day.toString().padLeft(2, '0')}T23:59:59.999Z';
      cubit.loadDashboard(page: 0, daterange: {'from': from, 'to': to});
    } else if (cubit.state.filters?.project != currentProjectId) {
      // Project changed — reload with current filters
      cubit.loadDashboard(page: 0, isRefresh: true);
    }
  }
  ...
  Widget _buildMetricCards(
    ...
    return AppMetricCard(
      ...
      onTaps: [
        () => _cubit.handleMetricTap(0, null),
        () => _cubit.handleMetricTap(1, TextHelper.ertTeamAssigned),
        () => _cubit.handleMetricTap(2, TextHelper.ertTasksAssigned),
        () => _cubit.handleMetricTap(3, TextHelper.ertTaskCompleted),
      ],
    );
  }
  ...
  Widget _buildGraphContent(
    ...
    return SafeArea(
      ...
      onRefresh: () async =>
          await _cubit.loadDashboard(isRefresh: true),
        ...
        child: Column(
          ...
          children: [
            ...
            CaseListSectionWidget(
              ...
              searchBarKey: searchBarKey,
              onActionTap: () => _cubit.toggleViewType(),
              items: (data.result ?? [])
                  .map<CaseListItem>(
              ...
              paginationWidget: AppPaginationControls(
                totalPages: _cubit.getTotalPages(),
