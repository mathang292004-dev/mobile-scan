...
class ErTeamApproverPaginationControls extends StatelessWidget {
  const ErTeamApproverPaginationControls({super.key});
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<ErTeamApproverDashboardCubit,
      ...
      builder: (context, state) {
        ...
        final currentPage = (state.filters?.page ?? 0) +
            1; // API uses 0-based, UI uses 1-based
  ...
  List<Widget> _buildPageNumbers(
    ...
    int currentPage,
    int totalPages,
    ErTeamApproverDashboardCubit cubit,
  ) {
    ...
    if (currentPage == totalPages || currentPage == totalPages - 1) {
      ...
      buttons.add(
        PageNumberButton(
          ...
          onTap: () =>
              _changePageWithCurrentFilter(context, totalPages - 1),
        ),
      );
    }
