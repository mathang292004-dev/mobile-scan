...
class InvestigationDateTimeFormatter {
  ...
  static Duration calculateTaskDuration({
    ...
    if (_isCompletedStatus(normalizedStatus) && completedAt != null) {
    ...
    if (normalizedStatus == 'paused' || normalizedStatus == 'draft') {
      if (pausedAt != null) {
        final duration = pausedAt.difference(startedAt);
        final adjustedDuration = duration - pausedDuration;
        return adjustedDuration.isNegative ? Duration.zero : adjustedDuration;
      }
    ...
    if (normalizedStatus == 'inprogress') {
      ...
      // Wait, if it resumed, pausedAt should probably be null or we track resumedAt?
      // Assuming pausedAt is cleared/null when resumed, or we have a different mechanism.
      // But looking at logic: "if (pausedAt != null)" -> "Working time before the task was paused".
      // This implies if pausedAt is SET, it is treated as PAUSED or JUST RESUMED logic?
      // Actually, if status is 'inprogress', usually pausedAt should be null OR we need resumedAt.
      // Let's keep the logic exactly as copied from source to avoid breaking existing logic assumptions,
      // just renaming the class.
      if (pausedAt != null) {
        // Working time before the task was paused
        // This is the time the user was actively working on the task
        final workBeforePause = pausedAt.difference(startedAt) - pausedDuration;
        return workBeforePause.isNegative ? Duration.zero : workBeforePause;
      }
